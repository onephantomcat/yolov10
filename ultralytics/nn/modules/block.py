...
class SPDConv(nn.Module):
    ...
    用于解决小目标在跨步卷积（Stride Convolution）下采样过程中的特征丢失问题。
    """

    def __init__(self, c1, c2, dimension=1):
        """Initializes SPDConv with input/output channels."""
        super().__init__()
        self.d = 2  # default scaling factor
        # 空间转深度后，通道数变为 c1 * 4，然后通过卷积调整到 c2
        self.conv = Conv(c1 * 4, c2, 3, 1)

    def forward(self, x):
        """Forward pass for SPD-Conv."""
        b, c, h, w = x.shape
        # Space-to-Depth operation: (B, C, H, W) -> (B, C*4, H/2, W/2)
